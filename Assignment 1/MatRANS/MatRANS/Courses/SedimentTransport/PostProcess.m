% Post-processor for MaRANS Matlab k-omega + sediment transport model
% Programmed by David R. Fuhrman
clear all;

% Load data from simulation
OutFileName = 'Case1.mat'; 
load(OutFileName);
...
ihold = 1; % Time series: 0: hold off, 1: hold on 
           % (can be used to add results from multiple simulations to one figure)
marker = 'b-'; % Marker for time series plots

pvec_an = [1 1 1 0 0    0       1 1  ]; % Plot control for animation
...
nrepeat = 1; % No. of times to repeat animation


% Choose time indices to plot / animate
%ivec = 1:length(t); % Plot /animate all saved time indices 
nT = 36; ivec = length(t)-nT:length(t)-1; % Plot / animate last period
%ivec = ivec(1:10); % Uncomment this line to stop at omega*t=90 deg.
