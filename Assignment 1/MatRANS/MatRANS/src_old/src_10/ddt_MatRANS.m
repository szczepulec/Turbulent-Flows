...
%
% Programmed by Signe Schloer, Johanna Sterner, and David R. Fuhrman
...
W = f(1+ny:2*ny,1);
K = f(1+2*ny:3*ny,1);
C = f(1+3*ny:3*ny+nyc,1);

% Eliminate any negative values
K = max(K,0); W = max(W,0); C = max(C,0);
...
elseif iwave == 3 % Solitary wave acceleratino
  u_fs = U_1m*sech(omega*(t-t0)).^2; % t0 set in main_MatRANS.m
...
% Determine mobile bed roughness, if desired
% This feature is only experimental, and has not been tested!
if k_N0 == 0
   %k_N = (2 + 0.6.*abs(Shields).^2.5).*d; % Mobile bed roughness (Sumer et al. 1996)
   k_N = max(2.5,2.5.*abs(Shields)).*d; % Linear growth
   k_N = min(k_N,10*d); % Set upper limit
else
   k_N = k_N0; 
end
...
% Update roughness, if desired
if k_N0 == 0
   %k_N = (2 + 0.6.*abs(Shields).^2.5).*d; % Mobile bed roughness (Sumer et al. 1996)
   %k_N = max(2.5,2.5.*abs(Shields)).*d;
   k_N = max(2,2.5.*abs(Shields)).*d;
else
   k_N = k_N0; 
end
k_Np = max(U_f*k_N/nu,1e-8); % Wall roughness
...
if Shields >= 0 % Positive
    Shields_c = Shields_cU; % Uphill critical Shields parameter
else % Negative
    Shields_c = Shields_cD; % Downhill critical Shields parameter
end

% The C equation (suspended sediment concentration)
% Calculate the reference concentration
if susp == 1 % Do if suspended sediment calculation is desired
    Shields_rel = abs(Shields)-Shields_c; Shields_rel = max(Shields_rel,0);
    if icb == 1 %  Zyserman & Fredsoe cb
        %cb = 0.331*Shields_rel^1.75/(1+(0.331/0.46)*Shields_rel^1.75);
        cb = 0.331*Shields_rel^1.75/(1+(0.331/0.32)*Shields_rel^1.75);
        C(1,1) = real(cb);
    elseif icb == 2 % O'Donoghue & Wright cb
        cb = 0.264;
        C(1,1) = real(cb);
    elseif icb == 3 || icb == 12 % Engelund & Fredsoe cb and Pickup function 2
        p_s = (1+((pi*mu_d/6)/(Shields_rel+1e-16))^4)^-0.25;
        if abs(Shields) > Shields_c+pi*p_s*mu_d/6;
            lambda = 0.4*2*((Shields_rel-pi*p_s*mu_d/6)/(0.013*abs(Shields)*s))^0.5;
            %cb = 0.65/(1+1/lambda)^3;
            cb = 0.6/(1+1/lambda)^3;
        else
            lambda = 0;
            cb = 0;
	...
	cb = cmax.*(abs(Shields)-theta_crs)./(theta_sheet-theta_crs);
	cb = cb.*(abs(Shields)>theta_crs);
	cb = min(cb,cmax);
	C(1,1) = cb;
    elseif icb == 6 % Tanh
	theta_susp = 0.25; cmax = 0.264; theta_sf = 2.0;
	cb = cmax.*tanh(pi./theta_sf.*(abs(Shields)-theta_susp));
	cb = cb.*(abs(Shields)>theta_susp);
	C(1,1) = cb;
    elseif icb == 11 % Pickup function of van Rijn  
        if abs(Shields) > Shields_c
            gammaS = 0.00083*(abs(Shields)/Shields_c-1)^1.5;
            pickup = w_s*gammaS;
        else
            pickup = 0;
        end
    ...
    if Hind_set == 1 % Hindered settling 
        w_s = w_s0.*(1-C).^nhs; % w_s varies with concentration
	settling = gradient(w_s.*C,yc');
    else % Constant w_s
        settling = w_s.*dCdy; % w_s*dc/dy
    end
    ...
    % Final C equation
    dCdt = -(u(end-nyc+1:end).*dCdx + v(end-nyc+1:end).*gradient(C,yc')) + dCdt0; % Full dC/dt
    %dCdt = -(0.*u(end-nyc+1:end).*dCdx + v(end-nyc+1:end).*gradient(C,yc')) + dCdt0; % dC/dt, no x convection
    %dCdt = -(u(end-nyc+1:end).*dCdx + 0.*v(end-nyc+1:end).*gradient(C,yc')) + dCdt0; % dC/dt, no y convection
    %dCdt(end) = 0; % Forces C=0 at top boundary condition
else
    dCdt = zeros(nyc,1); % Otherwise, just set vector to zero
end

% Turbulence supression due to density gradients
if turb && iturbsup && susp
  if iturbsup == 1
     rho_m = rho.*s.*C + rho.*(1-C); % Density of fluid-sediment mixture
  ...
  elseif iturbsup == 2
     dCdy_b = (C(2)-C(1))./(yc(2)-yc(1)); % dCdy @ y=b
     C_m = [C(1) + dCdy_b.*(y(1:ib-1)' - y(ib)); C]; % Extrapolate concentration
...
if mod(nrhs,noutput) == 0
    ...
    if ioutplot % Also show plots, if desired (to turn off set to 'if 0')
      ...
      subplot(3,4,4); plot(C,yc); xlabel('C'); ylim([0 ym]);
      ...
      subplot(3,4,8); plot(dCdt,yc); xlabel('dC/dt (1/s)'); ylim([0 ym]);    
