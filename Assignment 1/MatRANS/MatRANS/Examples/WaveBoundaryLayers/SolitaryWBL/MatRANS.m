...
iwave = 3; % Wave signal, 0: Off, 1: 2nd-order Stokes signal, 2: Wave form of Abreu et al. (2010), 3: Solitary wave
...
elseif iwave == 3 % Solitary wave
  U_1m = 1.19; % Maximum free stream velocity (m/s)
  t0 = T; % Lag time (s)
end
...
% Time input
dt = T/90; % Output time interval (s)
...
% Sediment input
d = 5e-7; % Grain size (m)
k_N = 1.32e-6; % Nikuradse's equivalent sand grain roughness (m)
...
k_Um2 = 1e-9; nuT_nu = 1e-12; % Initial k/U_m^2 and nuT/nu
K(1:ny,1) = k_Um2.*U_1m.^2; W(1:ny,1) = K/nuT_nu/nu;
