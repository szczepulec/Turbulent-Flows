...
turb = 2; % 1: turbulence model 0: Off, 1: On (if 0 then a laminar simulation is made)
...
nu = 9.6e-7; % Fluid kinematic viscosity (m^2/s)

% Output file name
OutFileName = 'out_MatRANS.mat';

% Grid input
h_m = 62/1000; % Height of model depth (m)
ny = 100; % Numbers of grid points in the vertical direction
%y = linspace(0,h_m,ny); % Define grid, linear spacing
y = h_m.*logspace(-1.8,0,ny-1); y = y - y(1); y = [y h_m]; % Define grid, logarithmic stretching

% Wave input 
T = 5; % Wave period (s)
c = 5.025; % Celerity (if streaming=1, used to calculate d/dx = -1/c*d/dt) (m/s)
iwave = 0; % Wave signal, 0: Off, 1: 2nd-order Stokes signal, 2: Wave form of Abreu et al. (2010)
if iwave == 1 % Input for second-order Stokes signal, with free stream velocity: U_1m*sin(omega*t) - U_2m*cos(2*omega*t)
  U_1m = 1.21; % Primary amplitude of free stream velocity (m/s)
  U_2m = 0; % Amplitude of second harmonic free stream velocity (m/s)
elseif iwave == 2 % Input for wave form of Abreu et al. (2010)
  U_1m = 1.524; % Total amplitude of free stream velocity signal
...
% Current input
Px = -0.021^2/h_m; % Constant added to 1/rho*dpdx (for no current, just set to zero) (m/s^2)
% For pure current, this will equal -Uf^2/h_m at steady state

% Time input
dt = 10; % Output time interval (s)
...
% Sediment input
d = 7/1000; % Grain size (m)
k_N = 9.9/1000; % Nikuradse's equivalent sand grain roughness (m)
...
% Closure coefficients for transitional k-omega model (Wilcox 2006, p.206; disregard unless turb=2)
...
W(1:ny,1) = 100; % Omega (1/s)
K(1:ny,1) = 0.1.*W.*nu; % Turbulent kinetic energy density (m^2/s^2)
%load initialUKW; % From standard k-omega simulation (created using makeInitialConditions.m script)
