...
% Grid input
h_m = 0.25; % Height of model depth (m)
ny = 100; % Numbers of grid points in the vertical direction
%y = linspace(0,h_m,ny); % Define grid, linear spacing
y = h_m.*logspace(-3.2,0,ny-1); y = y - y(1); y = [y h_m]; % Define grid, logarithmic stretching
...
iturbsup = 2; % Suppression terms 0: Off, 1: Full buoyancy flux, 2: Leading-order component (more stabile)
sigma_p = 0.7; % Closure coefficient (only used if iturbsup=1)
n = 0.4; % Porosity (used to estimate maximum extrapolated fluid-sediment density or concentration)	 
