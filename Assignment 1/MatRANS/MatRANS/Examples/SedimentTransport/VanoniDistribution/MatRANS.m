...
% Grid input
h_m = 2; % Height of model depth (m)
ny = 100; % Numbers of grid points in the vertical direction
%y = linspace(0,h_m,ny); % Define grid, linear spacing
y = h_m.*logspace(-4.2,0,ny-1); y = y - y(1); y = [y h_m]; % Define grid, logarithmic stretching
...
% Current input
Px = -0.1^2/h_m; % Constant added to 1/rho*dpdx (for no current, just set to zero) (m/s^2)
...
icb = 3; % 1: Zyserman-Fredsøe cb, 2: O'Donoghue-Wright cb, 3: Engelund-Fredsøe cb, 4: Einstein cb
% 11: Pickup function (van Rijn), 12: Pickup function based on Engelund-Fredsøe cb
beta_s = 1.0; % Sediment diffusivity (eps_s=beta_s*nu_T) (also, inverse of Prandtl-Schmidt number, if 0: Use van Rijn (1984) correction
...
BedLoad = 1; % 1: Meyer-Peter Muller, 2: Engelund-Fredsoe (7.59), 
