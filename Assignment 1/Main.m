...
%% Mean Velocity Profile with Log-Law Fit
figure(2);  
...
%% Inner-Scaled profile in outer Coordinates
figure(3); 
grid on;
...
%% Comparison of exp profile and Van Driest Model
figure(4);  
semilogy(U_plus_calc, y_plus_calc, 'LineWidth', 2);
hold on;
...
%% Inner-scaled RMS Velocity Fluctuations for 0 < y+ < 100
figure(6);
...
%% Outer-Scaled RMS Velocity Fluctuations
figure(9); 
...
%% Comparison of Reynolds Shear Stressx
figure(15);
...
axes('Position', [0.535 0.35 0.35 0.35]); 
box on; 
hold on;
plot(EE, y/h, 'LineWidth', 2); 
plot(uv_EE, y/h, 'x', 'MarkerSize', 8, 'LineWidth', 2);
...
velocity_RANS = readmatrix('MatRANS_Output/velocity.csv');
energy_k_RANS = readmatrix('MatRANS_Output/energy_k.csv');
yparameter = readmatrix("MatRANS_Output/yparameter.csv");

%% Comparison of Turbulent Kinetic Energy:
figure(20);
plot(k / U_f_calc^2,y / h,'x','MarkerSize', 8,'LineWidth', 2);
hold on;
...
%% Comparison of Mean Velocity Profile:
figure(21);
plot(u_mean / U_f_calc ,y / h,'x','MarkerSize', 8,'LineWidth', 2);
hold on;
