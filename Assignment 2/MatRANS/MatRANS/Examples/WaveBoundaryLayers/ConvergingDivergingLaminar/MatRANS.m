...
turb = 0; % 1: turbulence model 0: Off, 1: On (if 0 then a laminar simulation is made)
...
nu = 1e-3; % Fluid kinematic viscosity (m^2/s)

% Output file name
OutFileName = 'out_MatRANS.mat';

% Grid input
h_m = 1; % Height of model depth (m)
ny = 100; % Numbers of grid points in the vertical direction
%y = linspace(0,h_m,ny); % Define grid, linear spacing
y = h_m.*logspace(-2,0,ny-1); y = y - y(1); y = [y h_m]; % Define grid, logarithmic stretching

% Wave input 
T = 2*pi; % Wave period (s)
c = 10; % Celerity (if streaming=1, used to calculate d/dx = -1/c*d/dt) (m/s)
...
if iwave == 1 % Input for second-order Stokes signal, with free stream velocity: U_1m*sin(omega*t) - U_2m*cos(2*omega*t)
  U_1m = 1.0; % Primary amplitude of free stream velocity (m/s)
  U_2m = 0; % Amplitude of second harmonic free stream velocity (m/s)
elseif iwave == 2 % Input for wave form of Abreu et al. (2010)
  U_1m = 0.845; % Total amplitude of free stream velocity signal
  r = 2*0.57-1; % Controls nonlinearity, recommended range 0<=r<=0.75
         % If r=0, then the signal will be sinusoidal
  phi = 0; % Controls velocity asymmetry (phi=0) or skewness (phi=-pi/2)
end

% Bed slope effects (applied if S~=0)
S = 0.10; % -dh/dx, modifies critical Shields parameter in up/down hill directions
iconv = 1; % Converging-diverging effects 0: Off, 1: On
...
iturbsup = 1; % Suppression terms 0: Off, 1: On
