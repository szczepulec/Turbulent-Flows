...
% Grid input
h_m = 10; % Height of model depth (m)
ny = 100; % Numbers of grid points in the vertical direction
%y = linspace(0,h_m,ny); % Define grid, linear spacing
y = h_m.*logspace(-5,0,ny-1); y = y - y(1); y = [y h_m]; % Define grid, logarithmic stretching

% Wave input 
T = 8; % Wave period (s)
c = 5.02; % Celerity (if streaming=1, used to calculate d/dx = -1/c*d/dt) (m/s)
...
% Current input
Px = -(0.0157^2)/h_m; % Constant added to 1/rho*dpdx (for no current, just set to zero) (m/s^2)
%Px = -(0.0267^2)/h_m; % Constant added to 1/rho*dpdx (for no current, just set to zero) (m/s^2)
...
t2 = 2000*T; % End time (s)
t_span = [0:T:t2]; % Vector of time levels where output will be saved (s)

% Sediment input
d = 1/1000/2.5; % Grain size (m)
...
beta_s = 1; % Sediment diffusivity (eps_s=beta_s*nu_T) (also, inverse of Prandtl-Schmidt number, if 0: Use van Rijn (1984) correction
