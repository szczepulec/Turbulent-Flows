...
turb = 2; % 1: k-omega turbulence model 0: Off (laminar), 1: Standard, 2: Transitional
...
% Grid input
h_m = 60/1000; % Height of model depth (m)
ny = 100; % Numbers of grid points in the vertical direction
%y = linspace(0,h_m,ny); % Define grid, linear spacing
y = h_m.*logspace(-1.6,0,ny-1); y = y - y(1); y = [y h_m]; % Define grid, logarithmic stretching
...
% Current input
Px = -0.016^2/h_m; % Constant added to 1/rho*dpdx (for no current, just set to zero) (m/s^2)
...
w_s = 0.000001; % Fall velocity (if 0, then calculated empirically) (m/s)
...
main_MatRANS; % Location of sim_MatRANS.m must be in Matlab path
