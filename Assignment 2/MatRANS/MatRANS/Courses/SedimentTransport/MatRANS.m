...
% Grid input
h_m = 0.2; % Height of model depth (m)
ny = 60; % Numbers of grid points in the vertical direction
%y = linspace(0,h_m,ny); % Define grid, linear spacing
y = h_m.*logspace(-3.5,0,ny-1); y = y - y(1); y = [y h_m]; % Define grid, logarithmic stretching

% Wave input 
T = 8; % Wave period (s)
c = 5.25; % Celerity (if sec=1, used to calculate d/dx = -1/c*d/dt) (m/s)
iwave = 2; % Wave signal, 1: Second-order Stokes signal, 2: Wave form of Abreu et al. (2010)
if iwave == 1 % Input for second-order Stokes signal, with free stream velocity: U_1m*sin(omega*t) - U_2m*cos(2*omega*t)
  U_1m = 1.27; % Primary amplitude of free stream velocity (m/s)
  U_2m = 0.31; % Amplitude of second harmonic free stream velocity (m/s)
elseif iwave == 2 % Input for wave form of Abreu et al. (2010)
  U_1m = 1.27; % Total amplitude of free stream velocity signal
...
depth = 3.0; % Physical depth or tunnel half-depth
...
mu_d = 1.6; % Dynamic friction coefficient
...
BedLoad = 2; % 1: Meyer-Peter Muller, 2: Engelund-Fredsoe (7.59), 
...
beta_s = 1.0; % Sediment diffusivity (eps_s=beta_s*nu_T), if 0: Use van Rijn (1984) correction
...
iturbsup = 0; % Suppression terms 0: Off, 1: Full term, 2: Leading-order component
sigma_p = 0.7; % Additional closure coefficient
n = 0.4; % Porosity (used to estimate maximum fluid-sediment density)	 


% New cases for Sediment Transport Report 2 assignment (uncomment exactly one of the lines below!)
d=0.30/1000; r=0; phi=0; streaming=0; OutFileName='Case1.mat'; % Sinusoidal case
%d=0.30/1000; r=0; phi=0; streaming=0; Px=0.005; OutFileName='Case2.mat'; % Negative current
%d=0.30/1000; r=0; phi=0; streaming=1; Px=0; OutFileName='Case3.mat'; % Streaming
%d=0.30/1000; r=0.75; phi=-pi/2; streaming=0; Px=0; OutFileName='Case4.mat'; % Skewness
%d=0.30/1000; r=0.75; phi=0; streaming=0; Px=0; OutFileName='Case5.mat'; % Asymmetry
%d=0.30/1000; r=0.75; phi=-pi/4; streaming=1; Px=0.005; OutFileName='Case6.mat'; % All effects simultaneously
%d=0.13/1000; k_N=2.5*d; b=2*d; r=0.75; phi=-pi/2; streaming=0; Px=0; OutFileName='Case7.mat'; % Skewness, fine sand
%d=0.13/1000; k_N=2.5*d; b=2*d; r=0.75; phi=-pi/2; streaming=1; Px=0; OutFileName='Case8.mat'; % Skewness, fine sand, w /streaming
...
noutput = 500; % Displays time after every noutput RHS evaluations
