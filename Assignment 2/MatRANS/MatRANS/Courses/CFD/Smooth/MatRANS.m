...
% Create grid
h_m = ; % Height of model domain (m)
dy0 = ; % Near-wall grid size (m)
...
% Current body force input
Px = ; % Constant added to 1/rho*dpdx (for no current, just set to zero) (m/s^2)
% For pure current, this will equal -Uf^2/h_m at steady state

% Set roughness
k_N = ; % Nikuradse's equivalent sand grain roughness (m)
...
if iwave == 1 % Input for second-order Stokes signal, with free stream velocity: U_1m*sin(omega*t) - U_2m*cos(2*omega*t)
  U_1m = 0.0; % Primary amplitude of free stream velocity (m/s)
...
% Time input
dt = 10; % Time increment to save (s)
t_span = [0:dt:600]; % Vector of time levels where output will be saved (s)

% Sediment input
d = k_N/2.5; % Grain size (m)
%k_N = 0.0000795; % Nikuradse's equivalent sand grain roughness (m)
